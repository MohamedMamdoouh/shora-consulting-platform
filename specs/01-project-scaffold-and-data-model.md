...
**Booking/slot consistency invariant:** `Booking.AvailabilitySlotId` is the historical relationship; `AvailabilitySlot.BookingId` is the current holder pointer used for fast availability checks. They are updated in one DB transaction:

- reserve: set `Booking.Status = PendingPayment`, `Booking.AvailabilitySlotId = slotId`, `AvailabilitySlot.IsBooked = true`, `AvailabilitySlot.BookingId = bookingId`;
- release/cancel/fail: set `AvailabilitySlot.IsBooked = false`, `AvailabilitySlot.BookingId = null` while keeping the cancelled booking's `AvailabilitySlotId` for history (until/unless the slot row is later removed, in which case `SET NULL` applies and the booking's `SlotStartUtc`/`SlotEndUtc` snapshots carry the history).
  To prevent two concurrent active holders on one slot, enforce a **filtered unique index** on `Booking(AvailabilitySlotId)` for active statuses (`PendingPayment`, `PendingApproval`, `Confirmed`, `CancellationRequested`, `Completed`) where `AvailabilitySlotId IS NOT NULL`, allowing any number of historical `Cancelled` rows.
...
| AvailabilitySlotId       | `Guid?`                                                                                            | FK to the originally reserved slot, `ON DELETE SET NULL` — see slot-history rule below                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
- Exactly one active holder invariant remains: filtered unique index on `Booking(AvailabilitySlotId)` for active statuses.
...
- **Initial migration** (`Shora.Infrastructure/Persistence/Migrations/20260706205915_InitialCreate`) creates Identity tables + all domain tables above, including:
  ...
  - Filtered unique index on active `Booking(AvailabilitySlotId)` (statuses: `PendingPayment`, `PendingApproval`, `Confirmed`, `CancellationRequested`, `Completed`)
