...
Status: **Implemented** — JWT access tokens, HTTP-only refresh cookies, email verification, password reset, Admin role gating for `/admin/*` endpoints, and auth rate limits (spec 08).

## 1. Approach

- **ASP.NET Core Identity** manages users, roles, password hashing, external-login linking, and email confirmation.
- **Sign-in method**: **email + password**. No phone-based signup and no phone/OTP verification.
- **Email verification is required before booking**: unverified accounts can log in and browse, but cannot reserve a slot (enforced at `POST /api/bookings`, spec 04).
...
Applies to email/password accounts.
...
- Social login providers other than email/password (e.g. Facebook, Apple)
...
- `POST /api/auth/login`, `/signup`, `/forgot-password`, `/reset-password`, `/verify-email`, `/resend-verification`: limited **per IP** (e.g. a small number of attempts per minute).
