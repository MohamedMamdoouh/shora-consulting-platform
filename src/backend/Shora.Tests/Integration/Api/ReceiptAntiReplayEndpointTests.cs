...
public class ReceiptAntiReplayEndpointTests : IDisposable
{
    ...
    public ReceiptAntiReplayEndpointTests(PostgresFixture sqlServer)
    {
    ...
    public async Task Upload_receipt_flags_duplicate_content_hash_across_bookings()
    {
            ...
            "01011111111"))
        {
            ...
            var firstBody = await firstResponse.Content.ReadApiJsonAsync<UploadReceiptResponse>(cancellationToken);
        ...
        var secondBody = await secondResponse.Content.ReadApiJsonAsync<UploadReceiptResponse>(cancellationToken);
