...
@Component({
  ...
  template: `
    <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
      @if (gradient()) {
        <defs>
          <linearGradient [attr.id]="gradientId" x1="4" y1="18" x2="20" y2="4" gradientUnits="userSpaceOnUse">
      ...
      <path
        [attr.fill]="gradient() ? 'url(#' + gradientId + ')' : 'currentColor'"
        fill-rule="evenodd"
...
export class BrandMarkComponent {
  ...
  protected readonly gradientId = `shora-mark-gradient-${BrandMarkComponent.nextId++}`;
}
